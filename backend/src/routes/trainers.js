...
// Get all trainers with stats, grouped by canonical_id
router.get('/', async (req, res) => {
  try {
    ...
    const query = `
      ...
      LEFT JOIN race_records r ON r.trainer_id IN (
        SELECT id FROM trainers WHERE canonical_id = t.canonical_id
      )
      GROUP BY t.canonical_id, t.name_zh
...
// Get filter options for trainers
